...
const WorkoutCard = ({ workout, onStart, completed = false }) => {
  ...
  return (
    <motion.div
      ...
      className={`rounded border p-4 transition-all ${
        completed ? 'bg-success/5 border-success/15 opacity-60' : 'glow-card'
      }`}
    >
      ...
      <div className="flex items-center gap-4 text-xs text-text-muted mb-4">
        ...
        <span className="flex items-center gap-1 font-mono"><FiZap className="text-secondary" size={12} /> +{xpReward || 10} XP</span>
        {skillReward > 0 && (
      ...
      {!completed && (
        <button
          ...
          className="w-full py-2 bg-primary/10 text-secondary rounded border border-primary/20 text-xs font-system tracking-wider uppercase hover:bg-primary/20 hover:shadow-[0_0_10px_rgba(0,212,255,0.1)] transition-all"
        >
          Start Training
        </button>
      )}
